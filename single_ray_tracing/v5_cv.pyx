...
#			single_ray_lensing(x1tmp[i],xi2tmp[i],<Dtype *>spar.data,nspars,<Dtype *>spars_array.data,nssubs,<Dtype *>lpar.data,nlpars,<Dtype *>lpars_array.data,nlsubs,s_image[i],g_lensimage[i])
#	
#
#	cdef np.ndarray critical = np.zeros((nx1,nx2),dtype=np.double)
#	cdef np.ndarray caustic = np.zeros((nx1,nx2),dtype=np.double)
